...
void RenderingSystem::Init(GameCommon* owner)
{
	System::Init(owner);
}
...
void RenderingSystem::RegisterRenderObserver(RenderComponent* component)
{
	auto result = std::find(std::begin(m_renderObservers), std::end(m_renderObservers), component);
	if (result == std::end(m_renderObservers))
	{
		m_renderObservers.push_back(component);
	}
}

void RenderingSystem::UnregisterRenderObserver(RenderComponent* component)
{
	auto result = std::find(std::begin(m_renderObservers), std::end(m_renderObservers), component);
	if (result != std::end(m_renderObservers))
	{
		m_renderObservers.erase(result);
	}
}